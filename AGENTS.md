...
Protected surfaces are high-custody and licensable, not forbidden. Change them
only when the phase explicitly names the protected surface, scope, expected
behavior, tests, and validation boundary.
...
Stop and ask for product choices, unresolved architecture forks, unlicensed
protected-surface changes, live validation, secrets/private data, destructive
git, merge/rebase/force-push, broad scope expansion, or unresolved failing tests
that imply a design decision.
...
checks and results, protected surfaces, live-validation status, PR status, and
