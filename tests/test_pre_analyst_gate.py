...
from core.pipeline import run_economist_step as _actual_run_economist_step
...
class _PostEconomistHarness:
    def __init__(
        self,
        tmp_path: Path,
        *,
        report_type: str,
        economist_output: str,
        economist_telemetry: dict[str, Any] | None = None,
        healthy: bool = True,
        query: str = "What was Tesla's automotive gross margin trend over the last four reported quarters?",
        preflight_allows: bool = True,
        evidence_rows: list[dict[str, Any]] | None = None,
        router_entities: list[str] | None = None,
        use_actual_economist_step: bool = False,
        skip_quant_scout: bool = False,
    ) -> None:
        self.tmp_path = tmp_path
        self.report_type = report_type
        self.economist_output = economist_output
        self.economist_telemetry = economist_telemetry
        self.healthy = healthy
        self.query = query
        self.preflight_allows = preflight_allows
        self.evidence_rows = evidence_rows
        self.router_entities = router_entities
        self.use_actual_economist_step = use_actual_economist_step
        self.skip_quant_scout = skip_quant_scout
        self.analyst_calls = 0
        self.estimate_analyst_calls = 0
        self.thin_analyst_calls = 0
        self.economist_calls = 0
        self.analyst_prompts: list[str] = []
        self.author_prompts: list[str] = []
        self.author_system_prompts: list[str] = []
            ...
            return json.dumps(
                {
                    ...
                    "image_mode": "none",
                    "core_topic": self.query,
                    "is_academic": False,
                    "query_type": "quantitative_comparison"
                    if self.report_type in {"quantitative_comparison", "benchmark"}
                    else "product",
                    "entities": self.router_entities or ["Tesla"],
                    "primary_entity": (self.router_entities or ["Tesla"])[0],
                }
            )
        if system_prompt == "You are a concise title generator.":
            return "Tesla Margins"
        if system_prompt == DEFAULT_SYSTEM["researcher"]:
            return json.dumps({"queries": ["Tesla automotive gross margin 2025 quarters"]})
        if system_prompt.startswith("You are a search query expander"):
            return json.dumps({"component_queries": [], "reasoning": "retrieval has the quarter figures"})
        if system_prompt == DEFAULT_SYSTEM["evaluator"]:
            return json.dumps({"is_sufficient": True, "new_queries": []})
        if system_prompt.startswith("You classify evidence only."):
            return json.dumps({"Tesla": self.preflight_allows})
        if system_prompt == DEFAULT_SYSTEM["analyst"]:
            self.analyst_calls += 1
            self.analyst_prompts.append(prompt)
            return "Analysis: Tesla automotive gross margin trended from 18.5% to 20.1%."
        if system_prompt == DEFAULT_SYSTEM["analyst_estimate_from_priors"]:
            self.estimate_analyst_calls += 1
            self.analyst_prompts.append(prompt)
            return (
                "Estimate-from-priors analysis: evidence is thin, so provide "
                "uncertainty-bounded comparison logic."
            )
        if system_prompt == DEFAULT_SYSTEM["analyst_thin_quant"]:
            self.thin_analyst_calls += 1
            return "Thin anchor analysis: source anchors support the Economist framework."
        if system_prompt == DEFAULT_SYSTEM["synth_evaluator"]:
            return json.dumps({"is_sufficient": True, "supplemental_queries": []})
        if kwargs.get("stream"):
            self.author_system_prompts.append(system_prompt)
            self.author_prompts.append(prompt)
            return (
                "Tesla's automotive gross margin improved over the last four reported quarters, "
                "using the retrieved quarterly evidence and model-derived framework. "
                "[Tesla Q4 update](https://ir.tesla.com/q4)"
            )
        return "ok"
    ...
    def process_search_queries(self, queries: list[str], *_args: Any, **kwargs: Any) -> list[dict[str, Any]]:
        ...
        if self.evidence_rows is not None:
            out = []
            for idx, row in enumerate(self.evidence_rows):
                url = row.get("url") or f"https://example.test/source-{idx + 1}"
                if seen_urls is not None:
                    seen_urls.add(url)
                out.append(
                    {
                        "title": row.get("title") or f"Evidence source {idx + 1}",
                        "url": url,
                        "text": row.get("text") or "",
                        "score": row.get("score", 1.0 - (idx * 0.01)),
                        "credibility": row.get("credibility", 3),
                        "source_tier": row.get("source_tier", "official"),
                        "_provider": row.get("_provider", "tavily"),
                    }
                )
            return out
        out: list[dict[str, Any]] = []
        for idx, margin in enumerate(("18.5%", "18.9%", "19.3%", "20.1%")):
            if self.healthy:
                url = f"https://ir.tesla.com/q{idx + 1}"
                title = f"Tesla quarterly update {idx + 1}"
                text = (
                    f"Tesla automotive gross margin was {margin} in quarter {idx + 1}. "
                    "Tesla reported automotive revenue and automotive gross profit."
                )
                tier = "official"
            else:
                url = f"https://example-news.test/unrelated-{idx}"
                title = f"Unrelated market news {idx}"
                text = f"Unrelated company news with margin {margin}."
                tier = "unknown"
            if seen_urls is not None:
                seen_urls.add(url)
            out.append(
    ...
    def run_economist_step(self, *_args: Any, **_kwargs: Any) -> str:
        ...
        if self.use_actual_economist_step:
            _actual_run_economist_step(
                core_topic=_kwargs["core_topic"],
                all_passages=_kwargs["all_passages"],
                current_date=_kwargs["current_date"],
                ask_model=lambda *_model_args, **_model_kwargs: self.economist_output,
                clean_json_response=_kwargs.get("clean_json_response", lambda s: s),
                default_system=_kwargs.get("default_system", DEFAULT_SYSTEM),
                provider=_kwargs.get("provider", "OpenAI"),
                model=_kwargs.get("model", "gpt-5.4-mini"),
                base_url=_kwargs.get("base_url"),
                api_key=_kwargs.get("api_key"),
                scout_context=_kwargs.get("scout_context"),
                complexity=_kwargs.get("complexity", "medium"),
                corpus_weak=_kwargs.get("corpus_weak", False),
                corpus_state=_kwargs.get("corpus_state"),
                safety_telemetry=_kwargs.get("safety_telemetry"),
                user_query=_kwargs.get("user_query"),
            )
            return ""
        telemetry = _kwargs.get("safety_telemetry")
        if isinstance(telemetry, dict) and self.economist_telemetry is not None:
            telemetry.update(self.economist_telemetry)
        elif isinstance(telemetry, dict) and self.economist_output:
            telemetry.update(
                {
    ...
    def deps(self) -> RunDeps:
        return RunDeps(
            ...
            run_scout=lambda *_args, **_kwargs: {},
            should_skip_quant_scout=lambda *_args, **_kwargs: self.skip_quant_scout,
            clean_json_response=lambda s: s,
            DEFAULT_SYSTEM=DEFAULT_SYSTEM,
            NEWS_PREFERRED_DOMAINS=[],
            ACADEMIC_DOMAINS=[],
            QUANT_REPORT_TYPES={"quantitative_comparison", "benchmark"},
            logger=logging.getLogger("test_post_economist_gate"),
            execution_log_path=self.tmp_path / "execution.jsonl",
            feedback_log_path=self.tmp_path / "feedback.jsonl",
            kb_triggers_path=self.tmp_path / "kb.jsonl",
            policy_state_path=self.tmp_path / "policy.json",
...
def _run_post_economist_harness(
    tmp_path: Path,
    monkeypatch: Any,
    *,
    report_type: str,
    ...
    harness = _PostEconomistHarness(
        tmp_path,
        report_type=report_type,
        economist_output=economist_output,
        economist_telemetry=economist_telemetry,
        healthy=healthy,
        query=query,
        preflight_allows=preflight_allows,
        evidence_rows=evidence_rows,
        router_entities=router_entities,
        use_actual_economist_step=use_actual_economist_step,
        skip_quant_scout=skip_quant_scout,
    )
    outcome = run_pipeline(
        RunConfig(
            query=query,
            ...
            forced_corpus_state=(
                ...
                else (None if healthy else CorpusState.OFF_TOPIC.value)
            ),
        ),
        harness.deps(),
        NullStatusWriter(),
...
def _assert_author_prompt_has_no_raw_quant_material(
    prompt: str,
    *,
    allow_framework_not_run_note: bool = False,
) -> None:
    for marker in RAW_QUANT_AUTHOR_MARKERS:
        assert marker not in prompt
    assert "## QUANTITATIVE FRAMEWORK" not in prompt
    assert "QUANTITATIVE FRAMEWORK (MODEL-DERIVED" not in prompt
    if not allow_framework_not_run_note:
        assert "QUANTITATIVE FRAMEWORK" not in prompt
...
def test_shadow_economist_output_does_not_skip_full_analyst(
    tmp_path: Path, monkeypatch: Any
) -> None:
    ...
    assert harness.analyst_calls == 1
    assert harness.thin_analyst_calls == 0
    assert trace["analyst_skipped_after_economist"] is False
    assert trace["analyst_after_economist_skip_reason"] == "economist_shadow_mode_no_framework"
    assert trace["economist_output_used_as_analysis"] is False
    assert trace["economist_schema_valid"] is True
    assert trace["author_quant_content_source"] == "none"
    ...
    assert ECONOMIST_BLOCK not in harness.analyst_prompts[-1]
    assert "QUANTITATIVE FRAMEWORK" not in harness.analyst_prompts[-1]
    assert "ECONOMIST FRAMEWORK" not in harness.analyst_prompts[-1]
    assert harness.author_prompts
    assert ECONOMIST_BLOCK not in harness.author_prompts[-1]
    assert "QUANTITATIVE FRAMEWORK" not in harness.author_prompts[-1]
    assert "ECONOMIST FRAMEWORK" not in harness.author_prompts[-1]
    assert "quantitative_packet" not in harness.author_prompts[-1]


def test_abort_economist_response_does_not_skip_analyst_or_reach_author(
    tmp_path: Path, monkeypatch: Any
) -> None:
    outcome, harness = _run_post_economist_harness(
        tmp_path,
        monkeypatch,
        report_type="quantitative_comparison",
        economist_output="ABORT_ECONOMIST",
        use_actual_economist_step=True,
    )
    trace = outcome.execution_trace

    assert harness.economist_calls == 1
    assert trace["economist_ran"] is True
    assert trace["economist_schema_valid"] is False
    assert trace["economist_invalid_fields"] == []
    assert trace["quantitative_packet_present"] is False
    assert trace["quantitative_packet_valid"] is False
    assert trace["quantitative_packet_gate_reason"] == "packet_not_built"
    assert harness.analyst_calls == 1
    assert trace["analyst_model_called"] is True
    assert trace["analyst_skipped_after_economist"] is False
    assert trace["analyst_after_economist_skip_reason"] == "economist_empty_or_failed"
    assert trace["economist_output_used_as_analysis"] is False
    assert trace["author_quant_content_source"] == "none"
    assert trace["author_received_raw_quant_packet"] is False
    assert trace["author_received_economist_framework"] is False
    ...
    assert trace["economist_skip_eligibility_shadow_mode"] is True
    assert "packet_invalid_or_missing" in trace["economist_skip_eligibility_blockers"]
    assert harness.analyst_prompts
    assert "ABORT_ECONOMIST" not in harness.analyst_prompts[-1]
    assert harness.author_prompts
    assert "ABORT_ECONOMIST" not in harness.author_prompts[-1]
    _assert_author_prompt_has_no_raw_quant_material(harness.author_prompts[-1])


def test_malformed_economist_output_does_not_skip_analyst_or_reach_author(
    tmp_path: Path, monkeypatch: Any
) -> None:
    malformed_output = (
        '{"schema_version":"economist_v1",'
        '"quantitative_packet":{"schema_version":"quantitative_packet_v1"'
    )
    outcome, harness = _run_post_economist_harness(
        tmp_path,
        monkeypatch,
        report_type="quantitative_comparison",
        economist_output=malformed_output,
        use_actual_economist_step=True,
    )
    trace = outcome.execution_trace
    ...
    assert "packet_invalid_or_missing" in trace["economist_skip_eligibility_blockers"]
    assert harness.analyst_prompts
    assert malformed_output not in harness.analyst_prompts[-1]
    assert harness.author_prompts
    assert malformed_output not in harness.author_prompts[-1]
    _assert_author_prompt_has_no_raw_quant_material(harness.author_prompts[-1])


def test_invalid_source_binding_does_not_skip_analyst_or_reach_author(
    tmp_path: Path, monkeypatch: Any
) -> None:
    invalid_binding_telemetry = {
        "economist_schema_version": "economist_v1",
        "economist_schema_valid": True,
        "economist_invalid_fields": [],
        "unsupported_values_count": 0,
        ...
        "source_bound_value_count": 1,
        "source_binding_invalid_fields": [
            "unknown_source_id:source_bound_values[0].source_id"
        ],
        "source_binding_missing_source_id_count": 0,
        "source_binding_unknown_source_id_count": 1,
        "source_binding_malformed_count": 0,
        "source_ids_seen": ["1", "2", "3", "4"],
        "source_ids_used": ["999"],
        "source_binding_shadow_mode": True,
    ...
    outcome, harness = _run_post_economist_harness(
        ...
        report_type="quantitative_comparison",
        economist_output="",
        economist_telemetry=invalid_binding_telemetry,
    )
    trace = outcome.execution_trace
    ...
    assert harness.analyst_calls == 1
    assert harness.thin_analyst_calls == 0
    assert trace["source_binding_valid"] is False
    assert trace["source_binding_unknown_source_id_count"] == 1
    assert trace["analyst_skipped_after_economist"] is False
    assert trace["economist_output_used_as_analysis"] is False
    assert outcome.report.strip()
    assert harness.author_prompts and "QUANTITATIVE FRAMEWORK" not in harness.author_prompts[-1]


def test_non_quantitative_query_still_runs_analyst(tmp_path: Path, monkeypatch: Any) -> None:
    outcome, harness = _run_post_economist_harness(
        tmp_path,
        monkeypatch,
        report_type="general_research",
    ...
    assert harness.analyst_calls == 1
    assert trace["analyst_skipped_after_economist"] is False
    assert trace["analyst_after_economist_skip_reason"] == "report_type_not_bounded_quant"
    assert trace["economist_output_used_as_analysis"] is False


def test_empty_economist_output_does_not_skip_analyst(tmp_path: Path, monkeypatch: Any) -> None:
    outcome, harness = _run_post_economist_harness(
        tmp_path,
        monkeypatch,
...
def test_weak_off_topic_corpus_does_not_use_post_economist_skip(
    tmp_path: Path, monkeypatch: Any
) -> None:
    outcome, harness = _run_post_economist_harness(
        tmp_path,
...
def test_estimate_from_priors_currently_hits_pre_analyst_weak_gate_before_estimate_analyst(
    tmp_path: Path, monkeypatch: Any
) -> None:
    ...
    outcome, harness = _run_post_economist_harness(
        ...
        economist_output="",
        query=query,
        router_entities=["MD-80", "777-300ER"],
        forced_corpus_state=CorpusState.ESTIMATE_FROM_PRIORS.value,
    )
    trace = outcome.execution_trace

    assert trace["corpus_state"] == CorpusState.ESTIMATE_FROM_PRIORS.value
    assert trace["corpus_state_forced"] is True
    assert trace["corpus_weak"] is True
    assert trace["estimate_from_priors_requested"] is True
    assert trace["estimate_from_priors_blocked_by_pre_analyst_gate"] is True
    assert harness.economist_calls == 0
    assert trace["economist_ran"] is False
    assert trace["economist_preflight_allowed"] is None
    assert trace["analyst_skipped"] is True
    assert trace["analyst_skip_reason"] == "corpus_weak"
    assert trace["post_retrieval_fast_path_used"] is True
    assert harness.analyst_calls == 0
    assert harness.estimate_analyst_calls == 0
    ...
    assert trace["author_received_economist_framework"] is False
    assert trace["author_received_analyst_packet_marker"] is False
    assert trace["missing_target_metric_directive_emitted"] is False
    assert trace["author_system_prompt_key"] == "author_estimate_from_priors"
    assert harness.author_system_prompts
    assert harness.author_system_prompts[-1] == DEFAULT_SYSTEM["author_estimate_from_priors"]
    assert harness.author_prompts
    _assert_author_prompt_has_no_raw_quant_material(harness.author_prompts[-1])
    assert "economist_v1" not in harness.author_prompts[-1]
    assert "UNSUPPORTED_RETRIEVAL_DIRECTIVE" in harness.author_prompts[-1]


def test_post_economist_skip_telemetry_fields_present(tmp_path: Path, monkeypatch: Any) -> None:
    outcome, _harness = _run_post_economist_harness(
        tmp_path,
        monkeypatch,
        report_type="quantitative_comparison",
        economist_output=ECONOMIST_BLOCK,
    ...
    for key in (
        ...
        "economist_shadow_mode",
        "source_binding_valid",
        "source_bound_value_count",
        "source_binding_invalid_fields",
        "source_binding_missing_source_id_count",
        "source_binding_unknown_source_id_count",
        "source_binding_malformed_count",
        "source_ids_seen",
        "source_ids_used",
        "source_binding_shadow_mode",
        "calculation_requests_count",
        "calculation_success_count",
        "calculation_error_count",
        "unsupported_calculation_names",
        "calculation_input_binding_valid",
        "calculation_input_binding_error_count",
        "calculation_unresolved_input_refs",
        "calculation_results_count",
        "calculation_results_shadow_mode",
        "calculation_results",
        "target_metric_detected",
        "target_metric_names",
        "target_metric_evidence_found",
        "target_metric_bound_value_refs",
        "target_metric_calculation_refs",
        "target_metric_missing",
        "target_metric_shadow_would_block",
        "target_metric_gate_reason",
        "target_metric_shadow_mode",
        "high_stakes_quant_detected",
        "high_stakes_quant_domain",
        "high_stakes_quant_reasons",
        "high_stakes_quant_requires_analyst",
        "high_stakes_quant_shadow_mode",
        "high_stakes_quant_future_direct_use_allowed",
        "high_stakes_quant_gate_reason",
        ...
        "quant_retrieval_entity_coverage_valid",
        "quant_retrieval_metric_coverage_valid",
        "quant_retrieval_timeframe_coverage_valid",
        "quant_retrieval_comparison_coverage_valid",
        "quant_retrieval_source_diversity_count",
        "quant_retrieval_exact_value_binding_valid",
        ...
        "quant_retrieval_sufficiency_shadow_mode",
        "economist_pre_analyst_skip_candidate_shadow",
        "economist_pre_analyst_skip_candidate_reasons",
        "economist_pre_analyst_skip_candidate_blockers",
        "economist_pre_analyst_skip_candidate_gate_reason",
        "economist_pre_analyst_skip_candidate_shadow_mode",
        "analyst_quant_packet_present",
        "analyst_quant_packet_review_requested",
        "analyst_quant_packet_injected",
        "analyst_quant_packet_reviewed_by_model",
        "analyst_quant_packet_direct_use_eligible",
        "analyst_quant_packet_requires_analyst",
        "analyst_quant_packet_source",
        "analyst_quant_packet_gate_reason",
        ...
        "economist_skip_eligibility_shadow_mode",
    ...
    assert log_entry["analyst_skipped_after_economist"] is False
    assert log_entry["analyst_after_economist_skip_reason"] == "economist_shadow_mode_no_framework"
    assert log_entry["economist_output_used_as_analysis"] is False
    assert log_entry["source_binding_valid"] is True
    assert log_entry["source_ids_used"] == ["1"]
    assert log_entry["execution_trace"]["source_binding_shadow_mode"] is True
    assert "calculation_requests_count" in log_entry
    assert "calculation_results_count" in log_entry["execution_trace"]
    assert "target_metric_detected" in log_entry
    assert "target_metric_shadow_would_block" in log_entry["execution_trace"]
    assert "high_stakes_quant_detected" in log_entry
    assert "high_stakes_quant_gate_reason" in log_entry["execution_trace"]
    assert "quantitative_packet_present" in log_entry
    assert "quantitative_packet_gate_reason" in log_entry["execution_trace"]
    assert "quant_retrieval_sufficiency_valid" in log_entry
    assert "quant_retrieval_sufficiency_gate_reason" in log_entry["execution_trace"]
    assert log_entry["execution_trace"]["quant_retrieval_sufficiency_shadow_mode"] is True
    assert "economist_pre_analyst_skip_candidate_shadow" in log_entry
    assert "economist_pre_analyst_skip_candidate_gate_reason" in log_entry[
        "execution_trace"
    ]
    assert "analyst_quant_packet_reviewed_by_model" in log_entry
    assert "author_quant_content_source" in log_entry["execution_trace"]
    assert "economist_skip_eligible_shadow" in log_entry
    assert "economist_skip_eligibility_gate_reason" in log_entry["execution_trace"]
    assert "economist_skip_shadow_alignment" in log_entry
    assert "economist_evidence_source_ids_seen" in log_entry
    assert "final_answer_source_ids_used" in log_entry
    assert "final_answer_packet_source_ids_diverged" in log_entry["execution_trace"]


def test_quant_scout_skip_reason_records_numeric_evidence_skip(
    tmp_path: Path, monkeypatch: Any
) -> None:
    outcome, _harness = _run_post_economist_harness(
        tmp_path,
        ...
        report_type="quantitative_comparison",
        economist_output="",
        skip_quant_scout=True,
    )
    trace = outcome.execution_trace

    assert trace["scout_fired"] is False
    assert trace["scout_skip_reason"] == "numeric_evidence_sufficient"
    assert trace["author_system_prompt_key"] == "author"


def test_calculation_telemetry_does_not_skip_analyst_or_reach_author(
    tmp_path: Path, monkeypatch: Any
) -> None:
    ...
    outcome, harness = _run_post_economist_harness(
        ...
        report_type="quantitative_comparison",
        economist_output="",
        economist_telemetry=calculation_telemetry,
    )
    trace = outcome.execution_trace

    assert harness.economist_calls == 1
    assert harness.analyst_calls == 1
    assert trace["calculation_success_count"] == 1
    assert trace["calculation_results_count"] == 1
    assert trace["target_metric_evidence_found"] is True
    assert trace["target_metric_shadow_would_block"] is False
    assert trace["analyst_skipped_after_economist"] is False
    assert trace["economist_output_used_as_analysis"] is False
    assert harness.author_prompts and "QUANTITATIVE FRAMEWORK" not in harness.author_prompts[-1]


def test_missing_target_metric_telemetry_does_not_skip_analyst_or_reach_author(
    tmp_path: Path, monkeypatch: Any
) -> None:
    missing_price_telemetry = {
        "economist_schema_version": "economist_v1",
        "economist_schema_valid": True,
        "economist_invalid_fields": [],
        ...
        "target_metric_bound_value_refs": [],
        "target_metric_calculation_refs": [],
        "target_metric_missing": ["price_cost_rate"],
        "target_metric_shadow_would_block": True,
        "target_metric_gate_reason": "missing_target_metric_evidence",
        "target_metric_shadow_mode": True,
    }
    outcome, harness = _run_post_economist_harness(
        tmp_path,
        ...
        report_type="quantitative_comparison",
        economist_output="",
        economist_telemetry=missing_price_telemetry,
    )
    trace = outcome.execution_trace

    assert harness.economist_calls == 1
    assert harness.analyst_calls == 1
    assert trace["target_metric_detected"] is True
    assert trace["target_metric_evidence_found"] is False
    assert trace["target_metric_shadow_would_block"] is True
    assert trace["analyst_skipped_after_economist"] is False
    assert trace["economist_output_used_as_analysis"] is False
    assert harness.author_prompts and "QUANTITATIVE FRAMEWORK" not in harness.author_prompts[-1]


def test_high_stakes_quant_telemetry_does_not_skip_analyst_or_reach_author(
    tmp_path: Path, monkeypatch: Any
) -> None:
    high_stakes_telemetry = {
        "economist_schema_version": "economist_v1",
        "economist_schema_valid": True,
        "economist_invalid_fields": [],
        "unsupported_values_count": 0,
        ...
        "calculation_results": [
            {
                "name": "difference",
                "result": -0.9,
                "input_refs": {"a": "treatment_a_a1c", "b": "treatment_b_a1c"},
            }
        ],
        "target_metric_detected": True,
        "target_metric_names": ["comparative_terms"],
        "target_metric_evidence_found": True,
        "target_metric_bound_value_refs": [],
        "target_metric_calculation_refs": ["difference"],
        "target_metric_missing": [],
        "target_metric_shadow_would_block": False,
        "target_metric_gate_reason": "target_metric_supported_by_calculation",
        "target_metric_shadow_mode": True,
        ...
        "high_stakes_quant_domain": "medical",
        "high_stakes_quant_reasons": [
            "medical_domain_signal",
            "clinical_metric_signal",
            "decision_or_comparison_signal",
            "target_metric_detected",
            "calculation_requested",
            "calculation_result_present",
        ],
        "high_stakes_quant_requires_analyst": True,
        "high_stakes_quant_shadow_mode": True,
        "high_stakes_quant_future_direct_use_allowed": False,
    ...
    outcome, harness = _run_post_economist_harness(
        ...
        report_type="quantitative_comparison",
        economist_output="",
        economist_telemetry=high_stakes_telemetry,
    )
    trace = outcome.execution_trace
    ...
    assert trace["high_stakes_quant_domain"] == "medical"
    assert trace["high_stakes_quant_requires_analyst"] is True
    assert trace["high_stakes_quant_future_direct_use_allowed"] is False
    assert trace["analyst_skipped_after_economist"] is False
    assert trace["economist_output_used_as_analysis"] is False
    assert harness.author_prompts and "QUANTITATIVE FRAMEWORK" not in harness.author_prompts[-1]
    ...
    assert log_entry["high_stakes_quant_detected"] is True
    assert log_entry["execution_trace"]["high_stakes_quant_detected"] is True
    assert log_entry["execution_trace"]["high_stakes_quant_gate_reason"] == (
        "medical_quantitative_requires_analyst"
    )


def test_quantitative_packet_telemetry_does_not_skip_analyst_or_reach_author(
    tmp_path: Path, monkeypatch: Any
) -> None:
    packet_telemetry = {
        "economist_schema_version": "economist_v1",
    ...
    outcome, harness = _run_post_economist_harness(
        ...
        report_type="quantitative_comparison",
        economist_output="",
        economist_telemetry=packet_telemetry,
    )
    trace = outcome.execution_trace

    assert harness.economist_calls == 1
    assert harness.analyst_calls == 1
    assert trace["quantitative_packet_present"] is True
    assert trace["quantitative_packet_valid"] is True
    assert trace["quantitative_packet_direct_use_eligible"] is True
    assert trace["analyst_quant_packet_present"] is True
    assert trace["analyst_quant_packet_review_requested"] is True
    assert trace["analyst_quant_packet_injected"] is True
    assert trace["analyst_model_called"] is True
    assert trace["analyst_quant_packet_reviewed_by_model"] is True
    assert trace["analyst_quant_packet_direct_use_eligible"] is True
    assert trace["analyst_quant_packet_requires_analyst"] is False
    assert trace["analyst_skipped_after_economist"] is False
    assert trace["economist_output_used_as_analysis"] is False
    assert harness.analyst_prompts
    assert "QUANTITATIVE PACKET FOR ANALYST REVIEW ONLY" in harness.analyst_prompts[-1]
    assert "growth_change" in harness.analyst_prompts[-1]
    assert "old_revenue" in harness.analyst_prompts[-1]
    assert ECONOMIST_BLOCK not in harness.analyst_prompts[-1]
    assert "ECONOMIST FRAMEWORK" not in harness.analyst_prompts[-1]
    assert harness.author_prompts and "QUANTITATIVE FRAMEWORK" not in harness.author_prompts[-1]
    assert "ECONOMIST FRAMEWORK" not in harness.author_prompts[-1]
    assert "quantitative_packet" not in harness.author_prompts[-1]
    assert "Tesla automotive gross margin trended from 18.5% to 20.1%" in harness.author_prompts[-1]
    ...
    assert trace["economist_skip_eligible_shadow"] is False
    assert trace["economist_skip_eligibility_gate_reason"] == (
        "blocked_by_retrieval_sufficiency"
    )
    assert "retrieval_sufficiency_failed" in trace[
        "economist_skip_eligibility_blockers"
    ]
    assert trace["economist_skip_eligibility_shadow_mode"] is True
    assert trace["quant_retrieval_sufficiency_valid"] is False
    assert trace["quant_retrieval_sufficiency_shadow_mode"] is True
    assert trace["economist_pre_analyst_skip_candidate_shadow"] is False
    assert trace["economist_pre_analyst_skip_candidate_gate_reason"] == (
        "blocked_by_retrieval_sufficiency"
    )
    assert "retrieval_sufficiency_failed" in trace[
        "economist_pre_analyst_skip_candidate_blockers"
    ]
    assert trace["economist_skip_shadow_alignment"] == "neither"
    assert "analyst_reviewed_packet" in trace["economist_skip_eligibility_reasons"]
    assert "analyst_not_skipped" in trace["economist_skip_eligibility_reasons"]
    ...
    assert log_entry["quantitative_packet_present"] is True
    assert log_entry["analyst_quant_packet_injected"] is True
    assert log_entry["analyst_quant_packet_reviewed_by_model"] is True
    assert log_entry["author_quant_content_source"] == "analyst_reviewed"
    assert log_entry["execution_trace"]["quantitative_packet_valid"] is True
    assert log_entry["execution_trace"]["analyst_quant_packet_review_requested"] is True
    assert log_entry["economist_skip_eligible_shadow"] is False
    assert log_entry["economist_skip_eligibility_gate_reason"] == (
        "blocked_by_retrieval_sufficiency"
    )
    assert "retrieval_sufficiency_failed" in log_entry[
        "economist_skip_eligibility_blockers"
    ]
    assert log_entry["execution_trace"]["economist_skip_eligible_shadow"] is False
    assert log_entry["execution_trace"]["economist_skip_eligibility_gate_reason"] == (
        "blocked_by_retrieval_sufficiency"
    )
    assert log_entry["execution_trace"]["quant_retrieval_sufficiency_valid"] is False
    assert log_entry["execution_trace"][
        "economist_pre_analyst_skip_candidate_shadow"
    ] is False
    assert log_entry["economist_skip_shadow_alignment"] == "neither"


def test_clean_quantitative_packet_with_sufficient_retrieval_is_skip_eligible_shadow(
    tmp_path: Path, monkeypatch: Any
) -> None:
    ...
    packet_telemetry["quantitative_packet"].update(
        {
            "source_ids_used": ["1", "4"],
    ...
    assert trace["author_received_analyst_packet_marker"] is False
    assert harness.author_system_prompts
    assert harness.author_system_prompts[-1] == DEFAULT_SYSTEM["author"]
    assert harness.author_prompts
    _assert_author_prompt_has_no_raw_quant_material(harness.author_prompts[-1])
    assert "economist_v1" not in harness.author_prompts[-1]
    assert trace["quant_retrieval_sufficiency_valid"] is True
    assert trace["quant_retrieval_sufficiency_blockers"] == []
    assert trace["economist_pre_analyst_skip_candidate_shadow"] is True
    assert trace["economist_pre_analyst_skip_candidate_gate_reason"] == (
        "candidate_shadow_only"
    )
    assert trace["economist_pre_analyst_skip_candidate_blockers"] == []
    assert trace["economist_skip_eligible_shadow"] is True
    assert trace["economist_skip_eligibility_gate_reason"] == "eligible_shadow_only"
    assert trace["economist_skip_eligibility_blockers"] == []
    assert "retrieval_sufficiency_valid" in trace["economist_skip_eligibility_reasons"]
    assert trace["economist_skip_shadow_alignment"] == (
        "candidate_and_posthoc_eligible"
    )


def test_fast_mode_packet_injection_telemetry_does_not_claim_model_review(
    tmp_path: Path, monkeypatch: Any
) -> None:
    outcome, harness = _run_post_economist_harness(
        tmp_path,
        ...
        report_type="quantitative_comparison",
        economist_output="",
        economist_telemetry=copy.deepcopy(_valid_revenue_packet_telemetry()),
        mode="Fast",
    )
    trace = outcome.execution_trace
    ...
    assert trace["analyst_quant_packet_injected"] is True
    assert trace["analyst_model_called"] is False
    assert trace["analyst_quant_packet_reviewed_by_model"] is False
    assert trace["author_quant_content_source"] != "analyst_reviewed"
    assert trace["author_quant_content_source"] == "none"
    assert trace["economist_skip_eligible_shadow"] is False
    assert "packet_not_reviewed_by_analyst" in trace["economist_skip_eligibility_blockers"]
    assert "analyst_model_not_called" in trace["economist_skip_eligibility_blockers"]
    assert trace["economist_skip_eligibility_gate_reason"] == (
        "blocked_by_missing_analyst_review"
    )
    assert harness.author_prompts
    assert "QUANTITATIVE PACKET FOR ANALYST REVIEW ONLY" not in harness.author_prompts[-1]
    assert "quantitative_packet" not in harness.author_prompts[-1]


def test_fast_no_payload_high_stakes_medical_quantitative_query_records_skip_blockers(
    tmp_path: Path, monkeypatch: Any
) -> None:
    outcome, harness = _run_post_economist_harness(
        tmp_path,
        ...
        mode="Fast",
        query=(
            "Does semaglutide lower A1C more than metformin in adults with "
            "type 2 diabetes?"
        ),
    ...
    assert trace["high_stakes_quant_requires_analyst"] is True
    assert trace["high_stakes_quant_future_direct_use_allowed"] is False
    assert trace["high_stakes_quant_gate_reason"] == (
        "medical_quantitative_requires_analyst"
    )
    ...
    assert log_entry["high_stakes_quant_detected"] is True
    assert log_entry["execution_trace"]["high_stakes_quant_detected"] is True
    assert "high_stakes_requires_analyst" in log_entry["execution_trace"][
        "economist_skip_eligibility_blockers"
    ]


def test_high_stakes_quantitative_packet_is_injected_with_analyst_warning(
    tmp_path: Path, monkeypatch: Any
) -> None:
    packet_telemetry = copy.deepcopy(_valid_revenue_packet_telemetry())
    packet_telemetry.update(
    ...
    packet = packet_telemetry["quantitative_packet"]
    packet.update(
        {
    ...
    assert trace["analyst_model_called"] is True
    assert trace["analyst_quant_packet_reviewed_by_model"] is True
    assert trace["analyst_quant_packet_direct_use_eligible"] is False
    assert trace["analyst_quant_packet_requires_analyst"] is True
    assert trace["analyst_quant_packet_gate_reason"] == "high_stakes_requires_analyst"
    assert trace["analyst_skipped_after_economist"] is False
    assert trace["economist_output_used_as_analysis"] is False
    assert harness.analyst_prompts
    analyst_prompt = harness.analyst_prompts[-1]
    assert "QUANTITATIVE PACKET FOR ANALYST REVIEW ONLY" in analyst_prompt
    assert "MISSING TARGET METRIC EVIDENCE" not in analyst_prompt
    assert '"high_stakes_quant_detected":true' in analyst_prompt
    assert '"direct_use_eligible":false' in analyst_prompt
    assert '"requires_analyst":true' in analyst_prompt
    assert "a1c_change" in analyst_prompt
    assert harness.author_prompts and "quantitative_packet" not in harness.author_prompts[-1]
    assert "QUANTITATIVE FRAMEWORK" not in harness.author_prompts[-1]
    assert "MISSING TARGET METRIC EVIDENCE" not in harness.author_prompts[-1]
    assert trace["author_quant_content_source"] == "analyst_reviewed"
    assert trace["author_received_raw_quant_packet"] is False
    assert trace["author_received_analyst_packet_marker"] is False
    assert trace["author_received_economist_framework"] is False
    assert trace["economist_skip_eligible_shadow"] is False
    assert "high_stakes_requires_analyst" in trace["economist_skip_eligibility_blockers"]
    assert trace["economist_skip_eligibility_gate_reason"] == "blocked_by_high_stakes"


def test_invalid_quantitative_packet_is_not_injected_as_analyst_evidence(
    tmp_path: Path, monkeypatch: Any
) -> None:
    packet_telemetry = copy.deepcopy(_valid_revenue_packet_telemetry())
    packet_telemetry.update(
    ...
    packet_telemetry["quantitative_packet"]["validation_errors"] = [
    ...
    assert trace["quantitative_packet_present"] is True
    assert trace["quantitative_packet_valid"] is False
    assert trace["quantitative_packet_validation_errors"] == [
        "target_metric_evidence_missing"
    ]
    assert trace["analyst_quant_packet_present"] is True
    assert trace["analyst_quant_packet_review_requested"] is False
    assert trace["analyst_quant_packet_injected"] is False
    assert trace["analyst_model_called"] is True
    assert trace["analyst_quant_packet_reviewed_by_model"] is False
    assert trace["analyst_quant_packet_gate_reason"] == "packet_validation_failed"
    assert harness.analyst_prompts
    analyst_prompt = harness.analyst_prompts[-1]
    assert "QUANTITATIVE PACKET FOR ANALYST REVIEW ONLY" not in analyst_prompt
    assert "MISSING TARGET METRIC EVIDENCE" in analyst_prompt
    assert "Missing metric evidence: price_cost_rate" in analyst_prompt
    assert "Do not present model-derived numeric estimates, ranges, or percent advantages" in (
        analyst_prompt
    )
    assert trace["analyst_skipped_after_economist"] is False
    assert trace["economist_output_used_as_analysis"] is False
    assert trace["economist_skip_eligible_shadow"] is False
    assert "packet_invalid_or_missing" in trace["economist_skip_eligibility_blockers"]
    assert trace["economist_skip_eligibility_gate_reason"] == "blocked_by_invalid_packet"
    assert harness.author_prompts
    author_prompt = harness.author_prompts[-1]
    assert "MISSING TARGET METRIC EVIDENCE" in author_prompt
    assert "Missing metric evidence: price_cost_rate" in author_prompt
    assert "Do not present model-derived numeric estimates, ranges, or percent advantages" in (
        author_prompt
    )
    assert "Answer qualitatively from available sourced evidence" in author_prompt
    assert "quantitative_packet" not in author_prompt
    assert "QUANTITATIVE PACKET FOR ANALYST REVIEW ONLY" not in author_prompt
    assert "QUANTITATIVE FRAMEWORK" not in author_prompt
    assert "about 0.11" not in author_prompt
    assert "15-35%" not in author_prompt


def _proxy_only_defect_metric_evidence() -> list[dict[str, Any]]:
    return [
        {
            "title": "Alpha operating indicators",
            "url": "https://alpha.example/indicators",
            "text": (
                "Alpha support ticket volume was 480 in 2025 and product return "
                "volume was 32. The source does not publish the requested metric."
            ),
        },
        {
            "title": "Beta operating indicators",
            "url": "https://beta.example/indicators",
            "text": (
                "Beta support ticket volume was 510 in 2025 and product return "
                "volume was 30. The source does not publish the requested metric."
            ),
        },
    ]


def test_proxy_only_retrieval_missing_exact_metric_directs_analyst_and_author(
    tmp_path: Path, monkeypatch: Any
) -> None:
    outcome, harness = _run_post_economist_harness(
        tmp_path,
        ...
        report_type="quantitative_comparison",
        economist_output="",
        economist_telemetry=None,
        query="Compare Alpha vs Beta exact defect rate in 2025.",
        evidence_rows=_proxy_only_defect_metric_evidence(),
        router_entities=["Alpha", "Beta"],
    )
    trace = outcome.execution_trace

    assert harness.analyst_calls == 1
    assert trace["quant_retrieval_target_detected"] is True
    assert trace["quant_retrieval_metric_coverage_valid"] is False
    assert trace["quant_retrieval_proxy_metric_detected"] is True
    assert "proxy_metric_only" in trace["quant_retrieval_sufficiency_blockers"]
    assert "missing_metric_coverage" in trace["quant_retrieval_sufficiency_blockers"]
    assert trace["missing_target_metric_directive_emitted"] is True
    assert trace["author_system_prompt_key"] == "author"
    assert trace["analyst_skipped_after_economist"] is False
    assert trace["economist_skip_eligible_shadow"] is False
    ...
    analyst_prompt = harness.analyst_prompts[-1]
    assert "MISSING TARGET METRIC EVIDENCE" in analyst_prompt
    assert "Missing metric evidence: ratio_margin_share" in analyst_prompt
    assert "adjacent or proxy-only" in analyst_prompt
    assert "Do not present model-derived numeric estimates, ranges, or percent advantages" in (
        analyst_prompt
    )
    ...
    author_prompt = harness.author_prompts[-1]
    assert "MISSING TARGET METRIC EVIDENCE" in author_prompt
    assert "Missing metric evidence: ratio_margin_share" in author_prompt
    assert "Answer qualitatively from available sourced evidence" in author_prompt
    assert "quantitative_packet" not in author_prompt
    assert "economist_v1" not in author_prompt
    assert "QUANTITATIVE PACKET FOR ANALYST REVIEW ONLY" not in author_prompt
    assert "ECONOMIST FRAMEWORK" not in author_prompt


def test_soft_likely_exact_metric_request_still_emits_missing_metric_directive(
    tmp_path: Path, monkeypatch: Any
) -> None:
    outcome, harness = _run_post_economist_harness(
        tmp_path,
        ...
        economist_output="",
        economist_telemetry=None,
        query="Which of Alpha vs Beta likely had the lower exact defect rate in 2025?",
        evidence_rows=_proxy_only_defect_metric_evidence(),
        router_entities=["Alpha", "Beta"],
    )
    trace = outcome.execution_trace
    ...
    assert harness.analyst_prompts
    assert "MISSING TARGET METRIC EVIDENCE" in harness.analyst_prompts[-1]
    assert harness.author_prompts
    author_prompt = harness.author_prompts[-1]
    assert "MISSING TARGET METRIC EVIDENCE" in author_prompt
    assert "Do not present model-derived numeric estimates, ranges, or percent advantages" in (
...
def test_direct_exact_metric_evidence_does_not_emit_proxy_missing_directive(
    tmp_path: Path, monkeypatch: Any
) -> None:
    outcome, harness = _run_post_economist_harness(
        tmp_path,
        ...
        evidence_rows=[
            {
                "title": "Alpha quality report",
                "url": "https://alpha.example/quality",
                "text": "Alpha defect rate was 2.1% in 2025.",
            },
            {
                "title": "Beta quality report",
                "url": "https://beta.example/quality",
                "text": "Beta defect rate was 2.4% in 2025.",
            },
        ],
        router_entities=["Alpha", "Beta"],
    )
    ...
    assert harness.analyst_calls == 1
    assert trace["quant_retrieval_metric_coverage_valid"] is True
    assert trace["quant_retrieval_proxy_metric_detected"] is False
    assert "proxy_metric_only" not in trace["quant_retrieval_sufficiency_blockers"]
    assert trace["missing_target_metric_directive_emitted"] is False
    assert trace["author_system_prompt_key"] == "author"
    assert harness.analyst_prompts
    assert "MISSING TARGET METRIC EVIDENCE" not in harness.analyst_prompts[-1]
    assert harness.author_prompts
    assert "MISSING TARGET METRIC EVIDENCE" not in harness.author_prompts[-1]


def test_explicit_proxy_or_qualitative_request_does_not_emit_missing_metric_directive(
    tmp_path: Path, monkeypatch: Any
) -> None:
    outcome, harness = _run_post_economist_harness(
        tmp_path,
        ...
        economist_telemetry=None,
        query=(
            "Using proxy evidence only, qualitatively compare which of Alpha vs Beta "
            "likely has the lower defect rate in 2025."
        ),
        evidence_rows=_proxy_only_defect_metric_evidence(),
        router_entities=["Alpha", "Beta"],
    )
...
def test_preflight_blocked_economist_keeps_economist_ran_false(
    tmp_path: Path, monkeypatch: Any
) -> None:
    outcome, harness = _run_post_economist_harness(
        tmp_path,
        ...
        economist_output="",
        economist_telemetry=copy.deepcopy(_valid_revenue_packet_telemetry()),
        preflight_allows=False,
    )
    trace = outcome.execution_trace

    assert harness.economist_calls == 0
    assert trace["economist_ran"] is False
    assert trace["economist_preflight_allowed"] is False
    assert trace["economist_preflight_block_reason"] == (
        "missing_numerical_anchor_for_entities"
    )
    assert trace["economist_preflight_missing_entities"] == ["Tesla"]
    assert trace["quantitative_packet_present"] is False
    assert trace["quantitative_packet_valid"] is False
    assert trace["missing_target_metric_directive_emitted"] is False
    assert trace["analyst_quant_packet_injected"] is False
    ...
    assert trace["author_system_prompt_key"] == "author"
    assert harness.author_prompts
    assert "QUANTITATIVE FRAMEWORK NOT RUN" in harness.author_prompts[-1]
    assert "MISSING TARGET METRIC EVIDENCE" not in harness.author_prompts[-1]
    _assert_author_prompt_has_no_raw_quant_material(
        harness.author_prompts[-1],
        allow_framework_not_run_note=True,
    )


def test_non_quantitative_query_does_not_inject_analyst_quantitative_packet(
    tmp_path: Path, monkeypatch: Any
) -> None:
    outcome, harness = _run_post_economist_harness(
        tmp_path,
    ...
    assert harness.analyst_calls == 1
    assert trace["analyst_quant_packet_present"] is False
    assert trace["analyst_quant_packet_review_requested"] is False
    assert trace["analyst_quant_packet_injected"] is False
    assert trace["analyst_quant_packet_direct_use_eligible"] is False
    assert trace["analyst_quant_packet_requires_analyst"] is True
    assert trace["analyst_quant_packet_source"] is None
    assert trace["analyst_quant_packet_gate_reason"] == "no_packet_for_analyst"
    assert trace["analyst_model_called"] is True
    assert trace["analyst_quant_packet_reviewed_by_model"] is False
    assert trace["author_quant_content_source"] == "none"
    assert trace["author_received_raw_quant_packet"] is False
    ...
    assert trace["economist_skip_eligible_shadow"] is False
    assert "non_bounded_quantitative_report" in trace["economist_skip_eligibility_blockers"]
    assert trace["economist_skip_eligibility_gate_reason"] == "blocked_by_invalid_packet"
    assert harness.analyst_prompts
