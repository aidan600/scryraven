...
def test_balanced_pipeline_context_measurement_is_nested_hash_only(
    ...
    outcome, harness = run_post_retirement_ordinary_pipeline(
        ...
        healthy=True,
        forced_corpus_state="healthy",
        evidence_rows=evidence_rows,
        query="What was Tesla's automotive gross margin trend over four quarters?",
        core_topic="Tesla automotive gross margin",
        primary_entity="Tesla",
        router_entities=["Tesla"],
        researcher_queries=["Tesla automotive gross margin quarterly reports"],
    )
    row = execution_event_from_log(tmp_path / "execution.jsonl")
    ...
    assert "economist_preflight" not in measurement["stages"]
    ...
    assert DEFAULT_SYSTEM["economist"] not in harness.model_system_prompts
...
def test_context_measurement_preserves_balanced_provider_search_and_call_contracts(
    ...
    outcome, harness = run_post_retirement_ordinary_pipeline(
        ...
        researcher_queries=["Acme Widget pricing policy"],
    )
...
def test_context_measurement_is_diagnostic_only_for_fast_and_deep(
    ...
    fast_outcome, fast_harness = run_post_retirement_ordinary_pipeline(
        tmp_path / "fast", monkeypatch, mode="Fast"
    )
    deep_outcome, deep_harness = run_post_retirement_ordinary_pipeline(
        tmp_path / "deep", monkeypatch, mode="Deep"
    )
    ...
    assert fast_harness.economist_calls == []
    ...
    assert deep_harness.economist_calls == []
...
def test_context_measurement_sqlite_mapping_and_top_level_jsonl_stay_compact(
    ...
    _outcome, _harness = run_post_retirement_ordinary_pipeline(
