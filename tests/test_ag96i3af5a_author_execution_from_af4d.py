...
FORBIDDEN_TEXT = [REQUEST, "Bounded sanitized answer-bearing excerpt", "FINAL ANSWER PACKET AUTHORITY", RAW_RESPONSE_TAIL, "product output"]
...
class FakeAF5AAdapter:
    def __init__(self, response_text: str | None = None) -> None:
        self.response_text = response_text or (SAFE_CANDIDATE_PREFIX + " " + ("x" * af5a.MAX_BOUNDED_AUTHOR_RESPONSE_CANDIDATE_CHARS) + RAW_RESPONSE_TAIL)
    ...
    def __call__(
        ...
        *,
        ...
        self.calls.append(
            {"request_text": request_text, "request_digest": request_digest, "request_length": request_length, "request_metadata": deepcopy(request_metadata)}
        )
        return {
...
def test_af5a_happy_path_calls_fake_adapter_once_and_retains_bounded_candidate() -> None:
    ...
    assert action.inputs["af4d_author_model_request_assembly_id"] == (
        af4d_before["author_model_request_assembly_id"]
    )
    ...
    for surface in (state, projection):
        ...
        assert surface["author_execution_from_af4d_mode"] == af5a.AG96I3AF5A_AF4D_BOUND_AUTHOR_EXECUTION_LIVE_DISABLED_MODE
    ...
    assert candidate["bounded_sanitized_author_response_candidate_text"].startswith(
        SAFE_CANDIDATE_PREFIX
    )
    assert candidate["author_response_candidate_length"] == (
        af5a.MAX_BOUNDED_AUTHOR_RESPONSE_CANDIDATE_CHARS
    )
    assert candidate["author_response_candidate_digest"] == (
        state["author_response_candidate_digest"]
    )
    ...
    assert kernel.state.projections[af5a.FOLLOWUP_AUTHOR_EXECUTION_FROM_AF4D_STAGE] == (
        projection
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            lambda k: k.state.followup_author_model_request_assembly_projection.update(
...
@pytest.mark.parametrize(
    ...
    [
        (
            lambda k: k.state.followup_author_execution_from_ad_state.update(
                {"created": True}
            ),
            "old AE",
        ),
...
def test_af5a_static_guards_and_fast_custody_lane() -> None:
    ...
    forbidden_imports = set("core.author_execution_runtime core.followup_author_execution_from_ad_runtime core.runtime_prompt_assembly core.final_answer_runtime_assembly core.final_answer_runtime_adapter core.post_author_output_projection core.pipeline_orchestrator core.llm openai requests httpx urllib dotenv os subprocess".split())
    ...
    af5a_sections = [
        run_kernel_source.split("def authorize_followup_author_execution_from_af4d", 1)[1].split("def authorize_followup_author_observation", 1)[0],
        run_kernel_source.split('observation.payload.get("followup_author_execution_from_af4d_state")', 1)[1].split("self.state.reduced_action_ids.add", 1)[0],
        run_kernel_source.split("elif action.action_type is ActionType.FOLLOWUP_AUTHOR_EXECUTION_FROM_AF4D:", 1)[1].split("elif action.action_type is ActionType.FOLLOWUP_AUTHOR_OBSERVATION", 1)[0],
    ]
    for token in "ask_model( execute_author_action( ActionType.AUTHOR_EXECUTE ObservationType.AUTHOR_OUTPUT_OBSERVED build_followup_author_execution_from_ad_record execute_followup_author_execution_from_ad_action author_execution_runtime core.llm pipeline_orchestrator final_answer_runtime build_ordered_sources".split():
