...
def test_explicit_override_cannot_promote_speculative_bridge() -> None:
    path = InferencePath(
        path_id="override-speculative",
        target_claim=_target(),
        premises=(_premise("premise-b", "source-b"),),
        bridges=(
            _bridge(
                "speculative-link",
                InferenceBridgeType.MODEL_ASSUMED_SPECULATIVE,
...
def test_explicit_override_cannot_promote_blocked_premise() -> None:
    path = InferencePath(
        path_id="override-blocked",
        target_claim=_target(),
        premises=(_premise("blocked-premise", "source-b", conflict_impact=PremiseConflictImpact.BLOCKS),),
        bridges=(_bridge(),),
        posture="inferred_from_sourced_premises",
        recommendation="may_state",
    )
...
def test_explicit_override_cannot_promote_fast_multi_premise_inference() -> None:
    path = InferencePath(
        path_id="override-fast",
        target_claim=_target(),
        premises=(_premise("premise-b", "source-b"), _premise("premise-c", "source-c")),
        bridges=(_bridge(allowed_modes=(InferenceModePolicy.FAST, InferenceModePolicy.BALANCED)),),
        mode=InferenceModePolicy.FAST,
...
def test_explicit_override_cannot_promote_balanced_multi_hop_inference() -> None:
    path = InferencePath(
        path_id="override-balanced",
        target_claim=_target(),
        premises=(_premise("premise-b", "source-b"),),
        bridges=(
            _bridge("bridge-1", InferenceBridgeType.MATHEMATICAL),
