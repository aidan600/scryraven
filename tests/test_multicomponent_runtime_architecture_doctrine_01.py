...
def test_scheduler_lease_and_concurrency_invariants_are_visible() -> None:
    ...
    for phrase in (
        ...
        "Component Analyst input packets include the repository-owned model-visible quantitative proposal contract and bounded ordered `component_evidence_set`, but never a `quantitative_source_catalog`",
        "component contract binds the exact component target and the exact ordered `component_evidence_set`",
        ...
        "packet-digest and exact evidence-set-digest authority",
        "Missing, incomplete, malformed, cross-run, stale, reordered, or inconsistent authority fails before reduction",
