...
def test_searchos_slice_a_is_installed_and_navigation_remains_active() -> None:
    ...
    for phrase in (
        ...
        "Retired Scout and PlannerRevision RunDeps fields, the inert `core.scout` stub, and empty RunKernel Scout/Revision/SearchWorkPlan carriers are physically absent",
        "The former Scout and PlannerRevision runtime modules are deleted",
        "Ordinary `run_pipeline()` does not read or accept those surfaces",
