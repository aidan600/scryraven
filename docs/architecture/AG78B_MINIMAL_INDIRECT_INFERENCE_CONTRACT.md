...
The schema version is `AG78B.minimal_indirect_inference_contract.v1`. The trace/controller key is `indirect_inference_contract`. Evaluator-derived `InferencePath.posture` and `InferencePath.recommendation` are authoritative: constructor values for those fields are compatibility inputs only and cannot upgrade invalid, blocked, speculative, or mode-disallowed paths.
