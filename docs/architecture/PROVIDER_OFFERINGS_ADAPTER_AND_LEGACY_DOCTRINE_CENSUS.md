...
| `--mvp-single-relation-live-dogfood-run` and provider-decision commands | Explicit dogfood/operator, not ordinary main pipeline | DISCOVER/comparison mechanics only; untrusted exact-URL extraction blocks before dispatch | Explicit confirmation plus current target-safety and provider-eligibility gates | Diagnostic packets only; no READ/custody activation and no automatic main-pipeline activation | CURRENT_RUNTIME, CURRENT_TEST, CURRENT_CANONICAL_DOC |
