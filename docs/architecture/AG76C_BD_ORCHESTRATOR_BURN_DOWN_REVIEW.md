...
| JSONL/SQLite/persistence/outcome packaging | lines 6969-7300 | orchestrator plus JSONL/db/RunOutcome helpers | future `core.run_outcome_persistence_handoff` | mechanical_candidate_for_extraction | JSONL/SQLite, RunOutcome, trace fields | controller state, evidence registry, stage ledger tests | log, DB, session, outcome parity | medium | Candidate B after RT | P1 |
