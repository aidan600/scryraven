...
| Ordinary local checkout, Windows sandbox, Git, push, draft PR, and post-merge posture | [Windows Sandbox Publication Rule](CODEX_LOCAL_WINDOWS_SANDBOX_PUBLICATION_RULE.md) |
| Explicitly licensed dedicated Cursor worktree exception | [Optional Cursor Local Windows Worktree Rule](CURSOR_LOCAL_WINDOWS_PHASE_EXECUTION_RULE.md) |
...
| Concern | Read first |
| --- | --- |
| AnalystOS target topology, semantic-role responsibilities, mode policy, recovery return flow, N=1 folding, synthesis posture, and D-prime retirement direction (`canonical:analystos-operating-model`) | [AnalystOS Operating Model](../architecture/ANALYSTOS_OPERATING_MODEL.md) |
...
| Installed QueryPlan job classes, first-wave and iterative SearchJudgment, InterpretationBinding, clarification, slot-local cursors, follow-up DISCOVER result admission, candidate continuity, candidate READ, semantic handoff, and the installed one-hop boundary | [SearchOS First-Wave And Iterative-Judgment Cutover](../architecture/SEARCHOS_FIRST_WAVE_AND_ITERATIVE_JUDGMENT_CUTOVER.md) and [ScryRaven Current State](../architecture/SCRYRAVEN_CURRENT_STATE.md) |
| Installed Boundary A existing-gap recovery, Boundary B Analyst-proposed searched-premise recovery, append-only lease/cycle/generation policy, replay-before-currentness, bounded inference, stopping convergence, and legacy authority retirement (`canonical:searchos-post-analysis-recovery-and-inference-direction`) | [SearchOS Post-Analysis Recovery And Inference Direction](../architecture/SEARCHOS_POST_ANALYSIS_RECOVERY_AND_INFERENCE_DIRECTION.md), [Multi-Component Synthesis Runtime Architecture](../architecture/MULTICOMPONENT_SYNTHESIS_RUNTIME_ARCHITECTURE.md), and [ScryRaven Current State](../architecture/SCRYRAVEN_CURRENT_STATE.md) |
| Installed component and synthesis D-prime role contracts (`canonical:dprime-role-contract`; selected retirement target in [AnalystOS Operating Model](../architecture/ANALYSTOS_OPERATING_MODEL.md)) | [D-prime Architecture](../architecture/DPRIME_ARCHITECTURE.md) |
