...
class FollowupAuthorExecutionFromAF4DModelAdapter(Protocol):
    def __call__(self, request_text: str, *, request_digest: str, request_length: int, request_metadata: Mapping[str, Any]) -> Mapping[str, Any] | str: ...
    ...
    **_: Any,
    ...
    return {
        ...
        **{target: af4d.get(source) for target, source in copied.items()},
        "af4d_author_model_request_section_refs": safe_mapping_sequence(
            af4d.get("author_model_request_section_refs")
        ),
        "af4d_author_model_request_ref_digest": _digest(
            safe_mapping(af4d.get("author_model_request_ref"))
        ),
        "af4d_author_model_request_projection_digest": _digest(af4d_projection),
        "af4d_author_model_request_history_projection_digest": _digest(
            af4d_history[-1] if af4d_history else {}
        ),
...
def validate_run_kernel_followup_author_execution_from_af4d_state(
    *,
    ...
    for condition, message in (
        ...
        (state.get("af4d_author_model_request_digest") == state.get("reconstructed_author_model_request_digest"), "AF5A request digest mismatch"),
    ):
...
def _validated_context(
    *,
    ...
    require(transient.request_digest == af4d["state"].get("author_model_request_digest"), "AF5A reconstructed request digest must match AF4D")
    require(transient.request_length == af4d["state"].get("author_model_request_length"), "AF5A reconstructed request length must match AF4D")
    require(list(transient.section_refs) == safe_mapping_sequence(af4d["state"].get("author_model_request_section_refs")), "AF5A reconstructed request sections must match AF4D")
    return {
        "action": action,
        "af4d": af4d["state"],
        "af4c": af4c["state"],
...
def _validate_af4d(runtime_inputs: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for condition, message in (
        ...
        (projection.get("author_model_request_digest") == state.get("author_model_request_digest"), "AF5A stale AF4D projection digest"),
        (projection.get("author_model_request_assembly_id") == state.get("author_model_request_assembly_id"), "AF5A stale AF4D projection id"),
    ):
        require(condition, message)
    return current
def _validate_af4c(
    ...
    for condition, message in (
        (state.get("status") == FOLLOWUP_AUTHOR_INVOCATION_CONSTRUCTED_STATUS, "AF5A requires constructed AF4C invocation"),
        (state.get("author_invocation_ready_for_model") is True, "AF5A requires model-ready AF4C"),
        (af4d_state.get("af4c_author_invocation_construction_id") == state.get("author_invocation_construction_id"), "AF5A AF4D/AF4C id mismatch"),
        (af4d_state.get("af4c_author_invocation_digest") == state.get("ag96i3_author_invocation_digest"), "AF5A AF4D/AF4C digest mismatch"),
        (af4d_state.get("af4c_author_invocation_projection_digest") == _digest(projection), "AF5A stale AF4C projection digest"),
...
def _validate_af4b2(
    ...
    for condition, message in (
        (state.get("status") == FOLLOWUP_AUTHOR_EVIDENCE_CONTENT_BOUND_STATUS, "AF5A requires bound AF4B2 evidence content"),
        (payload_digest == state.get("sanitized_author_evidence_content_payload_digest"), "AF5A stale AF4B2 content payload digest"),
        (af4d_state.get("af4b2_author_evidence_content_bridge_id") == state.get("author_evidence_content_bridge_id"), "AF5A AF4D/AF4B2 id mismatch"),
        (af4d_state.get("af4b2_author_evidence_content_bridge_digest") == state.get("content_bridge_digest"), "AF5A AF4D/AF4B2 digest mismatch"),
        (af4d_state.get("af4b2_author_evidence_content_projection_digest") == _digest(projection), "AF5A stale AF4B2 projection digest"),
        (af4d_state.get("sanitized_author_evidence_content_payload_digest") == payload_digest, "AF5A AF4D/AF4B2 content payload digest mismatch"),
        (af4c_state.get("af4b2_author_evidence_content_bridge_digest") == state.get("content_bridge_digest"), "AF5A AF4C/AF4B2 digest mismatch"),
        (af4c_state.get("sanitized_author_evidence_content_payload_digest") == payload_digest, "AF5A AF4C/AF4B2 content payload digest mismatch"),
    ):
        require(condition, message)
    return current
def _candidate_from_adapter_response(
    ...
    if isinstance(adapter_response, Mapping):
        raw_text = (
            adapter_response.get("candidate_text")
            or adapter_response.get("text")
            or adapter_response.get("content")
        )
    else:
...
def _candidate_from_observed(
    ...
    text = clean_text(candidate.get("bounded_sanitized_author_response_candidate_text"), limit=MAX_BOUNDED_AUTHOR_RESPONSE_CANDIDATE_CHARS + 1)
    require(text, "AF5A observed response candidate text")
    require(text == candidate.get("bounded_sanitized_author_response_candidate_text"), "AF5A observed response candidate text must be canonical")
...
def _candidate_from_text(context: Mapping[str, Any], text: str) -> dict[str, Any]:
    ...
    digest = _digest({"bounded_sanitized_author_response_candidate_text": text, "af4d_author_model_request_digest": action.get("af4d_author_model_request_digest")})
    return safe_json({
...
def _receipt_from_adapter_response(
    ...
    receipt = {
        ...
        "adapter_invocation_count": _bounded_int(metadata.get("adapter_invocation_count") or metadata.get("invocation_count") or 1),
...
def _transient_from_state(state: Mapping[str, Any]) -> TransientAuthorModelRequest:
    digest = clean_text(
        state.get("reconstructed_author_model_request_digest")
        or state.get("af4d_author_model_request_digest"),
        limit=160,
    )
    return TransientAuthorModelRequest(
        request_text="",
...
def _bounded_user_request_context(request: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for field, value in candidates:
        if text := clean_text(value, limit=2000):
            return {"source_field": field, "request_text": text, "request_digest": _digest({"field": field, "value": text}), "request_length": len(text)}
    return {}
def _transient_request_sections(
    *,
    ...
    excerpt_lines = [_transient_excerpt_line(excerpt) for excerpt in payload if clean_text(excerpt.get("sanitized_excerpt_text"), limit=1000)]
    ...
    return [{"section_name": name, "section_text": text, "section_digest": _digest({"section_name": name, "section_text": text}), "section_length": len(text)} for name, text in sections]
...
def _section_ref(section: Mapping[str, Any]) -> dict[str, Any]:
    return {"section_ref_id": f"af4d-author-model-request-section:{section.get('section_name')}", **_section_ref_with_digest(section), "section_text_retained": False}
