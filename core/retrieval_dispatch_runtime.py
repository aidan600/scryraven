...
def _append_optional_kwargs(
    ...
    if dispatch.prior_queries_for_similarity is not None:
        kwargs["prior_queries_for_similarity"] = list(
            dispatch.prior_queries_for_similarity
        )
...
@dataclass(frozen=True)
class MainRetrievalPassRequest:
    iteration: int
    query_source: str
    ...
    include_domains: Sequence[str]
    exclude_domains: Sequence[str]
    exa_domain_filter: Sequence[str] | None
    entity_hint: str | None
    router_query_preparation_state: Any
    retrieval_stop_decision: Any
    run_id: str | None
    retrieval_budget_facts: dict[str, Any]
    batch_dispatch_authorization_ref: dict[str, Any]
    source_class_recovery_action_ref: dict[str, Any]
    weak_corpus_recovery_ref: dict[str, Any]
    previous_retrieval_loop_contract_state: RetrievalLoopState | None
    prior_queries_for_similarity: Sequence[str] | None = None
    query_similarity_basis: str | None = None
...
def execute_main_retrieval_pass_runtime(
    ...
    *,
    ...
    descriptor = build_retrieval_pass_descriptor(
        ...
        query_similarity_basis=request.query_similarity_basis,
        prior_queries_for_similarity=request.prior_queries_for_similarity,
        retrieval_budget_facts=request.retrieval_budget_facts,
        batch_dispatch_authorization_ref=request.batch_dispatch_authorization_ref,
        source_class_recovery_action_ref=request.source_class_recovery_action_ref,
        weak_corpus_recovery_ref=request.weak_corpus_recovery_ref,
    )
    envelope = build_retrieval_execution_envelope(
        descriptor,
        include_domains=request.include_domains,
        exclude_domains=request.exclude_domains,
        exa_domain_filter=request.exa_domain_filter,
        entity_hint=request.entity_hint,
    )
    loop_state = build_retrieval_loop_state(
        router_query_preparation_state=request.router_query_preparation_state,
        pass_descriptor=descriptor,
        execution_envelope=envelope,
        retrieval_stop_decision=request.retrieval_stop_decision,
    ...
    if request.previous_retrieval_loop_contract_state is not None:
        for previous_summary in (
            request.previous_retrieval_loop_contract_state.pass_result_summaries
        ):
    ...
    pass_record = build_retrieval_pass_record(
        ...
        iteration=request.iteration,
        queries=request.current_queries,
...
def execute_main_retrieval_pass_from_scope(
    ...
    *,
    ...
    return execute_main_retrieval_pass_runtime(
        MainRetrievalPassRequest(
            iteration=values["iteration"],
            query_source=(
                router_state.query_preparation_provenance.get("query_source")
                or values["retrieval_provider_role"]
            ),
            ...
            exa_domain_filter=(
                values["ACADEMIC_DOMAINS"] if values["is_academic"] else None
            ),
            ...
            retrieval_budget_facts={
                "iteration": values["iteration"],
                "max_iterations": values["max_iterations"],
                "iterations_remaining_after_pass": max(
                    0, values["max_iterations"] - values["iteration"]
                ),
                "results_per_query": values["results_per_query"],
                "top_chunks": values["top_chunks"],
            },
            batch_dispatch_authorization_ref=values["retrieval_batch_dispatch_trace"],
            source_class_recovery_action_ref=values[
                "active_source_class_recovery_lifecycle"
            ],
            weak_corpus_recovery_ref={
                "weak_corpus_recovery_used": values["weak_corpus_recovery_used"],
                "weak_corpus_recovery_attempted": values[
                    "weak_corpus_recovery_attempted"
                ],
                "weak_corpus_recovery_decision": values[
                    "weak_corpus_recovery_decision"
                ],
            },
            previous_retrieval_loop_contract_state=values[
                "retrieval_loop_contract_state"
            ],
...
def source_class_recovery_context_from_scope(
    ...
    *,
    ...
    return SourceClassRecoveryRunnerContext(
        ...
        lifecycle_trace=values["active_source_class_recovery_lifecycle"],
        process_search_queries=values["process_search_queries"],
        ...
        status_container=values["status"],
        search_providers=(
            list(values["providers_by_iteration"][-1])
            if values["providers_by_iteration"]
            else []
        ),
        exa_domain_filter=(
            values["ACADEMIC_DOMAINS"] if values["is_academic"] else None
...
def execute_conflict_resolution_from_scope(
    ...
    *,
    ...
    return execute_conflict_resolution_action(
        decision,
        lifecycle_trace=values["active_conflict_resolution_lifecycle"],
