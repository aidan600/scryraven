"""Product-facing retained source/evidence admission status.
...
prints only source/evidence admission status from the current EvidenceLedger
fetch/read candidate-custody machinery. It does not produce answer prose or
...
BLOCKED_SOURCE_EVIDENCE_CONSUMER_MISSING = (
    "BLOCKED_SOURCE_EVIDENCE_CONSUMER_MISSING"
)
BLOCKED_SOURCE_EVIDENCE_ADMISSION = "BLOCKED_SOURCE_EVIDENCE_ADMISSION"
BLOCKED_SOURCE_EVIDENCE_RAW_PRIVATE = "BLOCKED_SOURCE_EVIDENCE_RAW_PRIVATE"
BLOCKED_OUTPUT_HYGIENE = "BLOCKED_OUTPUT_HYGIENE"
BLOCKED_CLOSED_SURFACE_VIOLATION = "BLOCKED_CLOSED_SURFACE_VIOLATION"

NEXT_BLOCKED_SURFACE = "citation/source-obligation readiness product consumption"
...
SOURCE_EVIDENCE_MACHINERY = (
    "core.evidence_ledger_lifecycle.reduce_fetch_read_content_packet_into_evidence_ledger"
)
EXPLICIT_NON_CLAIM = (
    "This phase does not prove semantic support, component coverage, citation "
    "eligibility/rendering, source-obligation satisfaction, SufficiencyReadiness, "
    "FinalAnswerPacket readiness, Author behavior, answerability, final-answer "
    "quality, or product correctness."
)
...
class LiveSourceEvidenceAdmissionStatusResult:
    decision: str
    output: str
...
class LiveSourceEvidenceAdmissionStatusError(ValueError):
    """Raised for unexpected source/evidence status-consumer failures."""


def build_live_source_evidence_admission_status(
    *,
    query: str,
    repo_root: str | Path,
) -> LiveSourceEvidenceAdmissionStatusResult:
    """Consume retained artifacts and return a CLI-safe admission status result."""
    ...
    except LiveSourceEvidenceAdmissionStatusError as exc:
        return _blocked_result(
            query=query,
    ...
    if summary_status:
        return _blocked_result(
            query=query,
            blocker=summary_status,
            detail="fetch/read summary failed structural verification",
        )
    ...
    try:
        ledger_projection = _reduce_source_evidence_admission(fetch_packet)
        admission_ref = _source_evidence_admission_ref(
            ledger_projection=ledger_projection,
            fetch_packet=fetch_packet,
        )
    except EvidenceLedgerCandidateCustodyError as exc:
        blocker = (
            BLOCKED_SOURCE_EVIDENCE_RAW_PRIVATE
            if _looks_raw_private_error(str(exc))
            else BLOCKED_SOURCE_EVIDENCE_ADMISSION
        )
        return _blocked_result(query=query, blocker=blocker, detail=str(exc))
    except Exception as exc:
        return _blocked_result(
            query=query,
            blocker=BLOCKED_SOURCE_EVIDENCE_ADMISSION,
            detail=f"source/evidence admission failed: {exc}",
        )
    ...
    if admission_ref.get("status") != "custody_created":
        return _blocked_result(
            query=query,
            blocker=BLOCKED_SOURCE_EVIDENCE_ADMISSION,
            detail="EvidenceLedger fetch/read custody was not visible",
        )
    if _admission_opens_closed_surface(admission_ref):
        return _blocked_result(
            query=query,
            blocker=BLOCKED_CLOSED_SURFACE_VIOLATION,
            detail="source/evidence admission opened a closed downstream surface",
        )
    ...
    output = format_live_source_evidence_admission_status(payload)
    ...
    return LiveSourceEvidenceAdmissionStatusResult(
...
def format_live_source_evidence_admission_status(payload: Mapping[str, Any]) -> str:
    """Format concise CLI status without answer prose or readable text."""
    ...
    return "\n".join(
        (
            "live source/evidence admission status",
            f"phase: {PHASE}",
            f"mode: {MODE}",
            ...
            f"source/evidence admission machinery: {payload.get('source_evidence_machinery')}",
            f"source/evidence admission status: {admission.get('status')}",
            f"source/evidence admission owner: {admission.get('owner')}",
            f"source/evidence admission schema: {admission.get('schema_version')}",
            (
                "source/evidence admission observation id: "
                f"{admission.get('observation_id')}"
            ),
            (
                "source/evidence admission packet id: "
                f"{admission.get('fetch_read_content_packet_id')}"
            ),
            (
                "source/evidence admission packet digest: "
                f"{admission.get('fetch_read_content_packet_digest')}"
            ),
            f"source/evidence admission ref digest: {admission.get('ref_digest')}",
...
def _reduce_source_evidence_admission(
    fetch_packet: Mapping[str, Any],
) -> dict[str, Any]:
    run_id = _required_text(fetch_packet.get("run_id"), "fetch/read packet run_id")
    request_id = _required_text(
        fetch_packet.get("request_id"),
        "fetch/read packet request_id",
    )
    kernel = RunKernel(
        RunState(
            run_id=run_id,
            request_id=request_id,
            request={"source": PHASE, "live_calls_allowed": False},
        )
    )
    return reduce_fetch_read_content_packet_into_evidence_ledger(
        run_kernel=kernel,
        fetch_read_content_packet=fetch_packet,
    )


def _source_evidence_admission_ref(
    *,
    ...
    observation_refs = _safe_list(ledger_projection.get("observation_refs"))
    first_record = _safe_mapping(records[0]) if records else {}
    first_observation = _safe_mapping(observation_refs[0]) if observation_refs else {}
    ref = _without_empty(
        {
            "status": (
                "custody_created"
                if custody.get("candidate_content_custody_visible") is True
                and _bounded_int(custody.get("custody_record_count")) > 0
                else "not_admitted"
            ),
            "owner": custody.get("owner"),
            "schema_version": custody.get("schema_version"),
            "trace_key": custody.get("trace_key"),
            "observation_id": first_observation.get("observation_id"),
            "observation_source": first_observation.get("source"),
            "fetch_read_content_packet_id": fetch_packet.get("packet_id"),
            "fetch_read_content_packet_digest": fetch_packet.get("packet_digest"),
            "custody_record_count": _bounded_int(custody.get("custody_record_count")),
            "readable_record_count": _bounded_int(custody.get("readable_record_count")),
            "candidate_content_custody_visible": custody.get(
                "candidate_content_custody_visible"
            ),
            "candidate_id": first_record.get("candidate_id"),
            "reference_id": first_record.get("reference_id"),
            "reference_digest": first_record.get("reference_digest"),
            "candidate_content_custody_is_semantic_support": custody.get(
                "candidate_content_custody_is_semantic_support"
            ),
            "citation_eligible": custody.get("citation_eligible"),
            "source_obligation_satisfied": custody.get("source_obligation_satisfied"),
            "source_obligation_candidate_ids_satisfy_requirements": custody.get(
                "source_obligation_candidate_ids_satisfy_requirements"
            ),
            "component_coverage_created": custody.get("component_coverage_created"),
            "sufficiency_decided": custody.get("sufficiency_decided"),
            "final_answer_packet_created": custody.get("final_answer_packet_created"),
            "author_input_created": custody.get("author_input_created"),
            "partial_answer_ready": custody.get("partial_answer_ready"),
            "product_correctness_claimed": custody.get("product_correctness_claimed"),
...
def _admission_opens_closed_surface(admission_ref: Mapping[str, Any]) -> bool:
    ...
    for key, value in flags.items():
        if key in _CLOSED_FALSE_KEYS and value is not False:
            return True
    return False
...
def _pass_payload(
    *,
    ...
    return {
        ...
        "source_evidence_machinery": SOURCE_EVIDENCE_MACHINERY,
...
def _blocked_result(
    *,
    ...
    output = "\n".join(
        (
            "live source/evidence admission status blocked",
            f"phase: {PHASE}",
            f"mode: {MODE}",
...
def _select_and_verify_lineage(
    *,
    ...
    if search_candidate_packet.get("packet_digest") != search_result_candidate_packet.get(
        ...
        raise LiveSourceEvidenceAdmissionStatusError(
...
def _required_text(value: Any, label: str) -> str:
    ...
    if not text:
        raise LiveSourceEvidenceAdmissionStatusError(f"{label} is required")
    return text
...
__all__ = [
    "LIVE_SOURCE_EVIDENCE_ADMISSION_STATUS_FLAG",
    "LiveSourceEvidenceAdmissionStatusError",
    "LiveSourceEvidenceAdmissionStatusResult",
    "build_live_source_evidence_admission_status",
    "format_live_source_evidence_admission_status",
