...
- Answer may derive quantitative results by deterministic arithmetic over numeric values in selected Evidence, explicit user premises, and its own prior calculation results. The calculation is derived computation, not Evidence or a new support basis; external factual inputs still require supporting Evidence citations, and missing external inputs cannot be supplied by arithmetic.
