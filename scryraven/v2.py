...
class V2Limits:
    ...
    def __post_init__(self):
        if self.semantic_attempts < 2 or self.external_attempts < 0 or self.seconds <= 0 or self.attention_characters < 65_536:
            raise ValueError("invalid_v2_limits")
...
def _run_turn(
    ...
    limits: V2Limits | None = None, retained_acquisitions=(), context=None,
    ...
    if not isinstance(limits, V2Limits):
        raise ValueError("v2_requires_v2_limits")
    budget = _Budget(limits, clock)
    role = ModelRole("gpt-5.6-luna", "medium")
    model = model or OpenAIModel(ModelConfig(role, role), cache_namespace="scryraven-v2")
    ...
    while True:
        ...
        if budget.semantic >= limits.semantic_attempts - 1:
            ...
            selected = active
            break
        packet = {**common, "phase": "v2_research", "working_understanding": understanding,
